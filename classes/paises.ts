...
const crear = (request, response) => {
    ...
    if (nombre){
        pool.query('INSERT INTO pais (nombre) VALUES ($1)',[nombre],
        (error, results) => {
            if (error) {
                throw error;
            }
...
const editar = (request, response) => {
    ...
    if (nombre) {
        pool.query('UPDATE pais SET nombre=$1 WHERE id=$2',[nombre, id],
        (error, results) => {
            if (error) {
                throw error;
            }
...
const eliminar = (request, response) => {
    ...
    pool.query('DELETE FROM pais WHERE id=$1', [id], (error, results) => {
        if (error) {
            throw error;
        }
...
const porId = (request, response) => {
    ...
    pool.query('SELECT * FROM pais WHERE id=$1', [id], (error, results) => {
        if (error){
            throw error;
        }
        response.status(200).json(results.rows);
