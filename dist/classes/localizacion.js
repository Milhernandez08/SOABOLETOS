...
const todos = (request, response) => {
    connection_1.pool.query('SELECT * FROM localizacion l INNER JOIN usuario u ON l.id_user=u.id INNER JOIN lote lo ON l.id_lote=lo.id WHERE eliminado=0', (error, results) => {
        if (error) {
            throw error;
        }
...
const porId = (request, response) => {
    ...
    connection_1.pool.query('SELECT * FROM localizacion  l INNER JOIN usuario u ON l.id_user=u.id INNER JOIN lote lo ON l.id_lote=lo.id WHERE id=$1', [id], (error, results) => {
        if (error) {
            throw error;
        }
        response.status(200).json(results.rows);
...
const editar = (request, response) => {
    ...
    if (id_user && id_lote && nombre && latitud && longitud) {
        connection_1.pool.query('UPDATE localizacion SET id_user=$1, id_lote=$2, nombre=$3, latitud=$4, longitud=$5 WHERE id=$6', [id_user, id_lote, nombre, latitud, longitud, id], (error, results) => {
            if (error) {
                throw error;
            }
...
const eliminar = (request, response) => {
    const id = parseInt(request.params.id);
    if (id) {
        connection_1.pool.query('UPDATE localizacion SET eliminado=1 WHERE id=$1', [id], (error, results) => {
            if (error) {
                throw error;
            }
